...
func TestLoadConfigUsesDefaults(t *testing.T) {
	...
	if config.discoveryTimeout != defaultDiscoveryWait || config.refreshInterval != defaultRefreshInterval || config.shutdownTimeout != defaultShutdownTimeout {
		t.Errorf("default timing = (%s, %s, %s)", config.discoveryTimeout, config.refreshInterval, config.shutdownTimeout)
	}
}
...
func TestLoadConfigReadsAndTrimsEnvironment(t *testing.T) {
	...
	if config.discoveryTimeout != 3*time.Second || config.refreshInterval != 30*time.Second || config.shutdownTimeout != 4*time.Second {
		t.Errorf("configured timing = (%s, %s, %s)", config.discoveryTimeout, config.refreshInterval, config.shutdownTimeout)
	}
}
